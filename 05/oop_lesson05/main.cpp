...
//template <typename T>
//const T max_t(const T &a, const T &b) {//что бы функция могла принимать не только примитивные типы данных, но и пользовательские
//    //они должна при нимать занчения по ссылке, так как значения не должны изменяться внутри этой функции
//    //эта ссылка должна быть константной. И возвращаемым объектом тоже может быть константная ссылка
//    return (a>b) ? a : b;
//}
